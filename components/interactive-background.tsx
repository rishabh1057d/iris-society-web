  ...
  const initializeParticles = useCallback(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      particles.push({
        ...
        color: `hsl(${235 + Math.random() * 25}, 78%, 62%)`,
  ...
  const handleClick = useCallback((e: MouseEvent) => {
    ...
    for (let i = 0; i < burstParticles; i++) {
      ...
      particlesRef.current.push({
        ...
        opacity: 0.8,
        color: `hsl(${235 + Math.random() * 25}, 80%, 68%)`,
        life: 0,
        maxLife: 60,
      })
    }
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0">
      ...
      {!isMobile && !isReducedMotion && (
        ...
        >
          <div className="w-full h-full rounded-full bg-gradient-to-r from-[#3230e0] to-[#5b59f0] opacity-20 blur-sm" />
        </motion.div>
      )}
      ...
      <AnimatePresence>
        {clickEffects.map((effect) => (
          ...
          >
            <div className="w-4 h-4 rounded-full border-2 border-[#3230e0]" />
          </motion.div>
        ))}
      </AnimatePresence>
            ...
            <motion.div
              ...
              style={{
                background: `radial-gradient(circle, ${
                  i % 2 === 0 ? "rgba(50, 48, 224, 0.28)" : "rgba(91, 89, 240, 0.22)"
                } 0%, transparent 70%)`,
                left: `${20 + i * 20}%`,
                top: `${30 + i * 15}%`,
              }}
